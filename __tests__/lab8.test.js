describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    // Once you have the button, you can click it and check the innerText property of the button.
      ...
      const jsonInnerText = await innerText.jsonValue();
      expect(jsonInnerText).toBe('Remove from Cart');
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    // Query select all of the <product-item> elements, then for every single product element
      ...
      const innerValue = parseInt(await innerText.jsonValue());
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    // Also check to make sure that #cart-count is still 0
      ...
      for (let i = 0; i < prodItems.length; i++) {
          ...
          const innerText = await button.getProperty('innerText');
          const jsonInnerText = await innerText.jsonValue();
          expect(jsonInnerText).toBe('Add to Cart');
      }
      const cartCount = await page.$('#cart-count');
      const innerText = await cartCount.getProperty('innerText');
      const innerValue = parseInt(await innerText.jsonValue());
